...
같은 서체·같은 크기인데도 파워포인트가 재는 줄 폭이 브라우저보다 넓다. 이 저장소는 예산을 꽉
채워 잡으므로 그 차이가 곧 **"한 줄 더"**가 되고, 높이가 고정된 상자에서는 넘치거나 잘린다.
실측 사례: 표지 제목 `인공지능의 미래와`가 383pt 상자 안에서 357pt(여유 7%)로 측정됐는데도
파워포인트는 `와`를 다음 줄로 내렸다. **얼마나 넓게 재는지는 예측할 수 없다.**

그래서 `scripts/pptx-fit-text.py`가 내보내기 뒤에 세 가지를 건다.

1. **작성자가 정한 줄바꿈은 렌더러가 못 늘리게 한다.** 상자 높이가 담는 줄 수와 본문의 명시적
   줄 수가 같으면 그 줄바꿈이 전부라는 뜻이므로 자동 줄바꿈을 끈다. 단, 끈 뒤 가장 긴 줄이
   슬라이드 밖으로 나갈 것 같으면(12% 여유로 판단) 끄지 않는다.
2. **소스 들여쓰기를 접는다.** HTML 은 줄 앞 공백을 공백 하나로 접지만 내보내기는 그대로 싣는다.
   소스에서 들여쓴 문단이 슬라이드에서도 들여쓰인 채로 나온다.
3. **이미 92% 이상 찬 상자에만 "넘치면 축소"를 건다.** 전부에 걸면 넘치지도 않는 상자까지
   렌더러가 줄여 카드마다 글자 크기가 들쭉날쭉해진다.

**PPTX 는 서체 이름만 적고 파일을 담지 않는다.** 그 서체가 없는 컴퓨터에서는 대체 서체로 다시
흐르므로, 렌더로 줄바꿈을 판정하려면 먼저 그 서체를 설치해야 한다. 이 저장소의 woff2 는
`fontTools` 로 ttf 로 바꿔 `~/.fonts/` 에 넣으면 된다.
