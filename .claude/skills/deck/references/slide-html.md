...
### text 엔진을 쓰려면 지켜야 하는 두 가지

`--probe-text`로 어느 덱이 통과하는지 볼 수 있다. 막히는 이유는 실측상 딱 두 부류다.

1. **모든 텍스트가 시맨틱 텍스트 태그 안에 있어야 한다.** `<p>`, `<h1>`~`<h6>`, `<ul>`, `<ol>`.
   맨 `<div>`에 넣은 글자는 **파워포인트에서 아예 사라진다.** 이 저장소가 "시맨틱 태그"를 규칙으로
   내걸면서도 라벨·캡션을 `<div>`로 쓰던 곳들이 여기서 걸렸다.
2. **텍스트 상자가 아래 가장자리에서 0.5인치(36pt) 이상 떨어져야 한다.** 파워포인트의 안전 여백
   규칙이다. 대부분의 스타일이 푸터·소스 캡션을 바닥에서 20~32pt에 두므로 여기서 걸린다.
   `.slide`의 아래 패딩을 36pt 이상으로 잡으면 통과하지만, **그만큼 `main`의 세로 예산이 줄어든다.**
   PPTX 편집 가능성을 원하면 예산을 세울 때부터 그 36pt를 빼고 시작한다.
