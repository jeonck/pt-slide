...
`slides-grab`으로 720pt × 405pt 시맨틱 HTML 슬라이드를 만든다. 이 스킬은 이 저장소의 덱들 — 한국어 발표 덱, 92장 스타일 견본, MLOps·데브옵스 주제의 영어 덱 여럿 — 을 실제로 만들며 부딪힌 것들을 담고 있다.

여기 적힌 규칙은 대부분 **한 번씩 실패해 본 결과**다. `validate`가 통과시킨 결함, 추정으로 잡았다가
어긋난 가로 예산, 폴백 서체를 재고 있던 계측, 렌더에서 아예 사라진 요소. 그래서 순서를 지키는 것보다
**각 단계가 왜 있는지**를 이해하고 판단하는 게 중요하다 — 상황이 다르면 다르게 판단하되, 그 이유를 남긴다.
...
덱 폴더에 `README.md`를 쓴다 — 구성, 스타일, **판단한 것과 그 이유**(수치를 안 쓴 이유, 자리표시자, 팔레트 확장), 재생성 명령. 루트 README의 덱 표에 한 줄 추가하고, 랜딩 페이지를 다시 만든다:

```bash
node scripts/build-index.mjs      # 루트 README의 덱 표 → index.html
```

랜딩 페이지는 그 표에서 생성된다. 표에 줄을 넣고 이걸 돌리지 않으면 사이트에 그 덱이 안 뜬다.
